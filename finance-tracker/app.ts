...
function renderLine(): void {
  ...
  // Cumulative net worth per month
...
function renderGoals(): void {
  ...
  goalsListEl.innerHTML = goals.map(g => {
    ...
    return `<div class="goal-item ${done ? "done" : ""}">
      ...
      <div class="budget-bar-bg">
        <div class="budget-bar-fill ${done ? "" : pct >= 80 ? "" : ""}" style="width:${pct}%;background:${done ? "var(--income)" : "var(--savings)"}"></div>
      </div>
...
function hashPin(pin: string): string {
  // Simple hash — not cryptographic but good enough for localStorage
  let h = 0;
...
function resetInactivity(): void {
  ...
  const stored = load<string>(PK, "");
  if (!stored) return;
  inactivityTimer = setTimeout(() => { showPinScreen(); }, 5 * 60 * 1000); // 5 min
}
...
function scheduleNotifications(): void {
  ...
  unique.forEach(t => {
    ...
    if (daysUntil === 1 || daysUntil === 3) {
      // Show immediate notification as reminder
...
form.addEventListener("submit", e => {
  ...
  // Auto-categorize if category not manually changed from default
  let category = categorySelect.value;
  const suggested = autoCategory(desc);
  if (suggested && category === "Food") category = suggested; // only override if still on default

  if (!desc || isNaN(amount) || amount <= 0 || !date) return;

  // Duplicate warning — same desc + amount within last 2 days
  ...
  if (dupExists) {
    toast(`⚠ Possible duplicate: "${desc}" ${peso(amount)} already exists recently`, "info");
  }
...
// Auto-categorize hint on description input
...
bulkDeleteBtn.addEventListener("click", async () => {
  ...
  const ok = await confirm(`Delete ${selectedIds.size} transactions?`);
  if (!ok) return;
  transactions = transactions.filter(t => !selectedIds.has(t.id));
  selectedIds.clear();
  save(SK, transactions);
  bulkBar.classList.add("hidden");
  render(); toast(`Deleted ${selectedIds.size || "selected"} transactions`, "info");
});
